...
const ArrangmentBox = styled(Box)(
  sx({
    display: 'flex',justifyContent: 'left',paddingTop: 6,paddingBottom: 6,
  }),);

  const MainBox = styled(Box)(
    sx({
      display: 'flex',paddingLeft:7,paddingRight:7,alignItems: 'center',width:'100%',
    }),);
  ...
  return (
    <div className="detailsPage">
     ...
     <ArrangmentBox sx={responsiveBox}>
      <Link style={linkStyle} to="/"><div style={buttonStyle}> <BasicButton  value="Back" icon={<KeyboardBackspaceOutlinedIcon />}/></div></Link>
      </ArrangmentBox>
      
      <MainBox sx={responsiveMainBox}>
      <div className="imgDiv"><Image value={mainFlag}/></div>
       <div ><DiscriptionTemplete buttonsnames={buttons} subParagraph1={subParagraph1} subParagraph2={subParagraph2}/></div>
      </MainBox>
      </ThemeProvider>
     </div>
  );
